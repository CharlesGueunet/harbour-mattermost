...
// TODO create codeline() - same as for codespan, but without span, inline code
...
tickhandler(MMIOT *f, int tickchar, int minticks, int allow_space, spanhandler spanner)
{
	...
	// TODO if tick == 3 - multiline code, else if tick == 1 inline code( no span )
...
text(Paragraph *pp, MMIOT *f)
{
	 ...
	 * of whitespace */
		...
		case '~':   if ( (f->flags & (MKD_NOSTRIKETHROUGH|MKD_TAGTEXT|MKD_STRICT)) || ! tickhandler(f,c,2,0, delspan) )
				Qchar(c, f);
			break;
			// TODO make two differend code - inline code, and multiline code
